...
tape('CLI', function (t) {
  ...
  t.test('no mode specified', function (st) {
    var spt = spawn(st, './dsolcjs test/' + daodir + '/Token.sol');
    spt.stderr.match(/^Invalid option selected/);
    spt.end();
  });

  t.test('--bin', function (st) {
    var spt = spawn(st, './dsolcjs --bin test/' + daodir + '/Token.sol');
    spt.stderr.empty();
    spt.succeeds();
    spt.end();
  });
  ...
  t.test('incorrect source source', function (st) {
    var spt = spawn(st, './dsolcjs --bin test/fixtureIncorrectSource.sol');
    spt.stderr.match(/^test\/fixtureIncorrectSource.sol:1:1: SyntaxError: Invalid pragma "contract"/);
    spt.end();
  });

  t.test('--abi', function (st) {
    var spt = spawn(st, './dsolcjs --abi test/' + daodir + '/Token.sol');
    spt.stderr.empty();
    spt.succeeds();
    spt.end();
  });
  ...
  t.test('--bin --abi', function (st) {
    var spt = spawn(st, './dsolcjs --bin --abi test/' + daodir + '/Token.sol');
    spt.stderr.empty();
    spt.succeeds();
    spt.end();
  });
  ...
  t.test('standard json', function (st) {
    ...
    var spt = spawn(st, './dsolcjs --standard-json');
